...
class Listado {
    ...
    public function getCvs($idFormatoListado, $aditionalFilter = '') {
        ...
        $em = new EntityManager($this->form->getConection());
        $em->query($this->getQuery());
        $rows = $em->fetchResult();echo $this->getQuery();print_r($rows);
        $nRegistros = $em->numRows();
        $em->desConecta();
        unset($em);
        ...
        $file = "tmp/" . md5(date('d-m-Y H:i:s')) . ".txt";
        $archivo = new Archivo($file);
        $archivoSubido = $archivo->creaYSube($cvsString, "docs/csv", date('Y-m-d_H:i:s'). ".txt");
