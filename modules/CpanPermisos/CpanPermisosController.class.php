...
/**
 * CONTROLLER FOR CpanPermisos
 * @author: Sergio Perez <sergio.perez@albatronic.com>
 * @copyright: INFORMATICA ALBATRONIC SL 
 * @date 08.12.2012 00:00:15

 * Extiende a la clase controller
 */
class CpanPermisosController extends Controller {

    protected $entity = "CpanPermisos";
        ...
        if ($this->values['permisos']['permisosModulo']['IN']) {
            switch ($this->request["METHOD"]) {
                ...
                case 'POST': //CREAR NUEVO REGISTRO
                    ...
                    unset($datos);
                    return $this->listPopupAction($this->request[$this->entity]['IdPerfil']);
                    break;
            }
        } else {
            return array(
    ...
    public function listPopupAction($idPerfil = '') {

        if ($idPerfil == '')
            $idPerfil = $this->request[2];

        $valor = new CpanPermisos();
        $valor->setIdPerfil($idPerfil);
        $lineas[] = $valor;

        $valores = $valor->cargaCondicion("Id", "IdPerfil='{$idPerfil}'", "SortOrder ASC");
        unset($valor);

        foreach ($valores as $valor) {
            $lineas[] = new CpanPermisos($valor['Id']);
        }
